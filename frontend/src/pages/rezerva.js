...
const Login = () => {
  ...
  const csrfFetched = useRef(false); // Prevent duplicate fetch calls
  useEffect(() => {
    if (csrfFetched.current) return;
    csrfFetched.current = true;

    fetch('http://localhost:8000/api/csrf-token/', { // Use 'localhost' here if possible.
      method: 'GET',
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch('http://localhost:8000/api/login/', { // Use 'localhost' here too.
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'X-CSRFToken': csrfToken, // Include the token retrieved from the cookie.
        },
